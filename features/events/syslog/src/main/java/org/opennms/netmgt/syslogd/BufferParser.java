...
public class BufferParser {
		...
		public final ParserState apply(ParserState state) {
			...
			while(true) {
				...
				try {
					...
					if (m_terminal) {
					    if (m_resultConsumer != null) {
					        String messageValue= chechIfStructuredMessage(getValue(state).toString());
					        ...
					        {
					            m_resultConsumer.accept(state, (R) messageValue.trim());
					        }
					    }
	...
	private String chechIfStructuredMessage(String stateValue) {
	    return stateValue;
	}
