 ...
 * @author Seth
 */
public class SyslogKafkaElasticsearch2Test extends AbstractSyslogTestCase {
   ...
   // @Test
    public void testMinionSyslogsOverKafkaToEsRest() throws Exception {
        ...
        InetSocketAddress minionSshAddr = testEnvironment.getServiceAddress(ContainerAlias.MINION_OTHER_LOCATION, 8201);
       ...
       // sendMessage(ContainerAlias.MINION, sender, 100);
        ...
        for (int i = 0; i < (numMessages / chunk); i++) {
            ...
            sendMessage(ContainerAlias.MINION_OTHER_LOCATION, sender, chunk,null);
            count += chunk;
            if (count % logEvery == 0) {
